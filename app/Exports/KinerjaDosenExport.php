...
class KinerjaDosenExport implements WithMultipleSheets, ShouldAutoSize
{
    protected $kinerja;
    protected $scores;

    public function __construct(KinerjaDosen $kinerja, array $scores)
    {
    ...
    {
        return [
            'Informasi' => new KinerjaInfoSheet($this->kinerja),
            'Pengajaran' => new KinerjaDetailSheet($this->kinerja->pengajaran, 'Pengajaran'),
            'Penelitian' => new KinerjaDetailSheet($this->kinerja->penelitian, 'Penelitian'),
            'Pengabdian' => new KinerjaDetailSheet($this->kinerja->pengabdian, 'Pengabdian'),
            'Penunjang' => new KinerjaDetailSheet($this->kinerja->penunjang, 'Penunjang'),
...
class KinerjaInfoSheet implements WithTitle, WithHeadings, WithMapping, FromCollection, WithStyles, ShouldAutoSize
{
    ...
    public function __construct(KinerjaDosen $kinerja)
    {
        $this->kinerja = $kinerja;
    }
    ...
    {
        return [
            ['Nama Dosen', $kinerja->dosen->name],
            ['NIP/NIDN', ($kinerja->dosen->nip ?? '-') . ' / ' . ($kinerja->dosen->nidn ?? '-')],
            ['Jabatan', $kinerja->dosen->jabatan ?? '-'],
            ['Pangkat', $kinerja->dosen->pangkat ?? '-'],
            [],
            ['Semester', $kinerja->semester->nama_semester],
            ['Tanggal Pengisian', optional($kinerja->tanggal_pengisian)->format('d F Y') ?? '-'],
            ['Tanggal Validasi', optional($kinerja->tanggal_validasi)->format('d F Y') ?? '-'],
            ['Validator', $kinerja->validator->name ?? '-'],
            [],
            ['Status Penilaian', $kinerja->status_penilaian],
        ];
    }

    public function collection()
    {
        return collect([$this->kinerja]);
    }
...
class KinerjaDetailSheet implements WithTitle, WithHeadings, WithMapping, FromCollection, WithStyles, ShouldAutoSize
{
    ...
    {
        // Header style
        $headerRow = $this->title === 'Penunjang' ? 4 : 4; // Adjust based on columns
        ...
        $sheet->getColumnDimension('B')->setWidth(30); // Main description column
